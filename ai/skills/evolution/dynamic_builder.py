...
class BuilderWorkspace:
    ...
    async def run_python(self, code: str, timeout: float = 120.0) -> dict[str, Any]:
        from ai.skills.runtime.executor_runtime import (
            ExecutionPolicy,
            run_python_probe,
        )
...
async def build_skill(
    ...
    *,
    ...
    final_dir = output_dir or (
        Path("searchos_workspace/generated_skills/deepresearch/access")
        / _skill_dir_name(host)
    )
...
async def _skill_smoke_passes(
    ...
    *,
    ...
    from ai.skills.runtime.executor_runtime import ExecutionPolicy, run_executor
